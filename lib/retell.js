...
function buildPrompt({ businessName, ownerName, services, location, calendarOwner }) {
  const owner = ownerName ? ownerName.split(' ')[0] : 'our team';
  const serviceList = services || 'home services';
  const area = location || 'the local area';
  const calName = calendarOwner || owner;

  return `You are a friendly AI receptionist for ${businessName}, a ${serviceList} company serving ${area}. Speak with natural warmth and excitement. Vary your tone and pace. Sound like a real, enthusiastic person — never flat or monotone. Always speak in a consistent American Midwest accent.
...
When someone calls:
- Greet them: "Thanks for calling ${businessName}, this is Sarah, how can I help you today?"
...
function buildTools(apiKey) {
  const sharedHeaders = { 'x-api-key': process.env.RETELL_TOOL_SECRET || apiKey || process.env.GHL_AGENCY_KEY };
 ...
 */
export async function createRetellAgent({ businessName, ownerName, services, location, ghlApiKey }) {
  if (!process.env.RETELL_API_KEY) {
    throw new Error('Missing RETELL_API_KEY');
  }

  const prompt = buildPrompt({ businessName, ownerName, services, location });
  ...
  const agent = await retellFetch('/create-agent', 'POST', {
    ...
    voice_speed: 0.9,
    language: 'en-US',
    webhook_url: process.env.RETELL_TOOL_SECRET
      ? `${process.env.NEXT_PUBLIC_APP_URL}/api/retell/call-start?secret=${process.env.RETELL_TOOL_SECRET}`
      : `${process.env.NEXT_PUBLIC_APP_URL}/api/retell/call-start`,
