...
/**
 * Fetch the location-level Private Integration Token (PIT) using the agency key.
 * Returns the key string (pit-xxx) or null on failure.
 */
export async function fetchLocationApiKey(locationId) {
  try {
    const data = await agencyFetch(`/locations/${locationId}/apiKey`);
    return data?.key ?? null;
  } catch (err) {
    console.error('[highlevel] fetchLocationApiKey error:', err.message);
    return null;
  }
}
 ...
 */
export async function createSubAccount({ name, email, phone, plan, businessName, snapshotId: snapshotIdOverride }) {
  const snapshotId = snapshotIdOverride || process.env.GHL_SNAPSHOT_ID;
  ...
  // ── Step 4: Fetch the location-level PIT key ────────────────────────────────
  const locationApiKey = await fetchLocationApiKey(locationId);
  if (locationApiKey) {
    console.log(`[highlevel] PIT key fetched for ${locationId}`);
  } else {
    console.warn(`[highlevel] could not fetch PIT key for ${locationId}`);
  }

  // ── Step 5: Auto-provision Retell agent for Rocket/Velocity plans ──────────
