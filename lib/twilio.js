...
async function findOrCreateContact(phone, locationId, apiKey) {
  ...
  if (!searchRes.ok) {
    throw new Error(`GHL contact search failed: ${searchRes.status}`);
  }
  ...
  if (!createRes.ok) throw new Error(`Failed to create contact: ${createData?.message || createRes.status}`);
  return createData?.contact?.id ?? createData?.meta?.contactId;
}
