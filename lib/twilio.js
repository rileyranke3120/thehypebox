...
export async function sendSMS(to, body, { apiKey, locationId } = {}) {
  const resolvedApiKey = apiKey || process.env.GHL_SMS_KEY || process.env.GHL_DAVE_API_KEY;
  const resolvedLocationId = locationId || process.env.GHL_DAVE_LOCATION_ID;

  if (!resolvedApiKey || !resolvedLocationId) {
    throw new Error('Missing GHL credentials for SMS — pass { apiKey, locationId } or set GHL_API_KEY / GHL_LOCATION_ID env vars');
  }
