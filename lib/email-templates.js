...
export function welcomeEmail({ name, email, password, plan, trialEndDate }) {
  ...
  const body = `
    <h1 style="font-size:1.75rem;font-weight:800;color:#fff;margin:0 0 8px;">
      You're in, ${esc(firstName)}! 🎉
    </h1>
    <p style="font-size:1rem;color:#999;margin:0 0 32px;">
      Your <strong style="color:${YELLOW};">${esc(planLabel)}</strong> 14-day free trial is now active.
    </p>
    ...
    <div style="background:#111;border:1px solid #222;border-radius:8px;padding:24px;margin-bottom:32px;">
      ...
      <div>
        ...
        <p style="font-size:1.2rem;color:${YELLOW};margin:0;font-family:monospace;font-weight:700;letter-spacing:0.1em;">${esc(password)}</p>
      </div>
    </div>
    ...
    <p style="font-size:0.82rem;color:#555;margin:32px 0 0;line-height:1.6;">
      Your trial runs until ${esc(trialEndDate)} — no charge until then.<br>
...
export function trialEndingEmail({ name, plan, planPrice, trialEndDate }) {
  ...
  const body = `
    ...
    <p style="font-size:1rem;color:#999;margin:0 0 32px;">
      Hi ${esc(firstName)}, your free trial of <strong style="color:#fff;">${esc(planLabel)}</strong> wraps up on ${esc(trialEndDate)}.
    </p>
    ...
    <div style="background:#111;border:1px solid #222;border-left:3px solid ${YELLOW};border-radius:4px;padding:24px;margin-bottom:32px;">
      ...
      <p style="font-size:0.95rem;color:#ccc;margin:0 0 8px;">
        ✓ First payment of <strong style="color:#fff;">$${esc(planPrice)}/mo</strong> on ${esc(trialEndDate)}
      </p>
      <p style="font-size:0.95rem;color:#ccc;margin:0 0 8px;">
...
export function paymentSuccessEmail({ name, plan, amountCents, nextBillingDate, invoiceUrl }) {
  ...
  const body = `
    ...
    <p style="font-size:1rem;color:#999;margin:0 0 32px;">
      Hi ${esc(firstName)}, we've successfully processed your payment for <strong style="color:#fff;">${esc(planLabel)}</strong>.
    </p>

    <div style="background:#111;border:1px solid #222;border-radius:8px;padding:24px;margin-bottom:32px;">
      <div style="display:flex;justify-content:space-between;margin-bottom:16px;">
        <span style="font-size:0.7rem;color:#555;text-transform:uppercase;letter-spacing:0.08em;">Amount Paid</span>
        <span style="font-size:1rem;color:${YELLOW};font-weight:700;">${esc(amount)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;margin-bottom:${invoiceUrl ? '16px' : '0'};">
        ...
        <span style="font-size:0.95rem;color:#ccc;">${esc(nextBillingDate)}</span>
      </div>
      ${invoiceUrl ? `
      <div style="display:flex;justify-content:space-between;">
        <span style="font-size:0.7rem;color:#555;text-transform:uppercase;letter-spacing:0.08em;">Invoice</span>
        <a href="${esc(invoiceUrl)}" style="font-size:0.95rem;color:${YELLOW};">View Receipt →</a>
      </div>` : ''}
    </div>
...
export function paymentFailedEmail({ name, plan, amountCents, updateUrl }) {
  ...
  const body = `
    ...
    <p style="font-size:1rem;color:#999;margin:0 0 32px;">
      Hi ${esc(firstName)}, we weren't able to process your payment of <strong style="color:#fff;">${esc(amount)}</strong> for <strong style="color:#fff;">${esc(planLabel)}</strong>.
    </p>
    ...
    <a href="${esc(updateUrl)}" style="display:inline-block;background:#E24B4A;color:#fff;font-weight:700;font-size:1rem;padding:14px 32px;border-radius:4px;text-decoration:none;">
...
export function highLevelAccessEmail({ name, plan, locationId, hlEmail, hlPassword, dashboardUrl, hasRetell = false }) {
  ...
  const body = `
    <h1 style="font-size:1.75rem;font-weight:800;color:#fff;margin:0 0 8px;">
      Your AI tools are ready, ${esc(firstName)}!
    </h1>
    <p style="font-size:1rem;color:#999;margin:0 0 32px;">
      We've set up your <strong style="color:${YELLOW};">${esc(planLabel)}</strong> account in our automation platform.
    ...
    <div style="background:#111;border:1px solid #222;border-radius:8px;padding:24px;margin-bottom:24px;">
      ...
      <div style="margin-bottom:12px;">
        <p style="font-size:0.7rem;color:#555;margin:0 0 4px;text-transform:uppercase;letter-spacing:0.08em;">Email</p>
        <p style="font-size:1rem;color:#fff;margin:0;font-family:monospace;">${esc(hlEmail)}</p>
      </div>
      ${hlPassword ? `
      <div style="margin-bottom:20px;">
        <p style="font-size:0.7rem;color:#555;margin:0 0 4px;text-transform:uppercase;letter-spacing:0.08em;">Temporary Password</p>
        <p style="font-size:1.2rem;color:${YELLOW};margin:0;font-family:monospace;font-weight:700;letter-spacing:0.1em;">${esc(hlPassword)}</p>
      </div>
    ...
    <a href="${esc(dashboardUrl)}" style="display:inline-block;background:${YELLOW};color:#000;font-weight:700;font-size:1rem;padding:14px 32px;border-radius:4px;text-decoration:none;margin-bottom:32px;">
      Open Your Platform →
    </a>
...
export function trialDripEmail({ name, plan, day, loginUrl, trialEndDate }) {
  ...
  const variants = {
    1: {
      ...
      heading: `Let's get you set up, ${esc(firstName)}`,
      body: `
        <p style="font-size:1rem;color:#999;margin:0 0 24px;">
          Your <strong style="color:${YELLOW};">${esc(planLabel)}</strong> trial is live. Here's what to do in the next 24 hours:
        </p>
        ...
        <a href="${esc(login)}" style="display:inline-block;background:${YELLOW};color:#000;font-weight:700;font-size:0.9rem;padding:12px 28px;border-radius:4px;text-decoration:none;letter-spacing:0.05em;">Go to dashboard →</a>
      `,
    },
    3: {
      ...
      body: `
        <p style="font-size:1rem;color:#999;margin:0 0 24px;">
          You've had your <strong style="color:${YELLOW};">${esc(planLabel)}</strong> for 3 days. How's it going?
        </p>
        ...
        <a href="${esc(login)}" style="display:inline-block;background:${YELLOW};color:#000;font-weight:700;font-size:0.9rem;padding:12px 28px;border-radius:4px;text-decoration:none;letter-spacing:0.05em;">Open dashboard →</a>
      `,
    },
    7: {
      ...
      body: `
        <p style="font-size:1rem;color:#999;margin:0 0 24px;">
          You're at the midpoint of your <strong style="color:${YELLOW};">${esc(planLabel)}</strong> trial.
        </p>
        <p style="font-size:0.9rem;color:#aaa;margin:0 0 24px;line-height:1.7;">
        ...
        <a href="${esc(login)}" style="display:inline-block;background:${YELLOW};color:#000;font-weight:700;font-size:0.9rem;padding:12px 28px;border-radius:4px;text-decoration:none;letter-spacing:0.05em;">Check your dashboard →</a>
      `,
    },
    10: {
      ...
      body: `
        <p style="font-size:1rem;color:#999;margin:0 0 24px;">
          4 days left on your trial, ${esc(firstName)}.
        </p>
        ${plan === 'launch' ? `
    ...
    13: {
      ...
      body: `
        <p style="font-size:1rem;color:#999;margin:0 0 24px;">
          Your <strong style="color:${YELLOW};">${esc(planLabel)}</strong> trial ends ${trialEndDate ? `on ${esc(trialEndDate)}` : 'tomorrow'}.
        </p>
        <p style="font-size:0.9rem;color:#aaa;margin:0 0 24px;line-height:1.7;">
        ...
        <a href="${esc(login)}" style="display:inline-block;background:${YELLOW};color:#000;font-weight:700;font-size:0.9rem;padding:12px 28px;border-radius:4px;text-decoration:none;letter-spacing:0.05em;">Manage your plan →</a>
      `,
    },
  };
...
export function subscriptionCanceledEmail({ name, plan, accessEndDate }) {
  ...
  const body = `
    ...
    <p style="font-size:1rem;color:#999;margin:0 0 32px;">
      Hi ${esc(firstName)}, your <strong style="color:#fff;">${esc(planLabel)}</strong> subscription has been canceled.
    </p>
...
// ── 7. Monthly Client Report ───────────────────────────────────
export function monthlyClientReportEmail({
  name,
  businessName,
  monthLabel,
  totalCalls,
  ...
  function metricCell(label, value, color) {
    return `
      <td style="padding:0 6px 12px 0;width:20%;vertical-align:top;">
        <div style="background:#111;border:1px solid #1f1f1f;border-radius:6px;padding:14px 10px;text-align:center;">
          <div style="font-size:0.6rem;color:#555;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:8px;line-height:1.3;">${esc(label)}</div>
          <div style="font-size:1.4rem;font-weight:800;color:${color};line-height:1;">${esc(String(value))}</div>
        </div>
      </td>`;
  }

  const bookRate = totalCalls > 0
    ? `${Math.round((appointmentsBooked / totalCalls) * 100)}%`
    : '—';

  const pipelineDisplay = pipelineValue != null
    ? '$' + Number(pipelineValue).toLocaleString()
    : '—';

  const body = `
    <div style="margin-bottom:6px;">
      <span style="font-size:0.7rem;color:#555;letter-spacing:0.12em;text-transform:uppercase;">${esc(monthLabel)} — Monthly Report</span>
    </div>
    <h1 style="font-size:1.5rem;font-weight:900;color:#fff;margin:0 0 6px;line-height:1.2;">
      ${esc(biz)}
    </h1>
    <p style="font-size:0.9rem;color:#888;margin:0 0 28px;">
      Hi ${esc(firstName)}, here's what your AI system did last month.
    </p>

    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;" role="presentation">
      <tr>
        ${metricCell('Calls Handled', totalCalls, '#FFD000')}
        ${metricCell('Booked', appointmentsBooked, '#1D9E75')}
        ${metricCell('Booking Rate', bookRate, appointmentsBooked > 0 ? '#1D9E75' : '#555')}
        ${metricCell('Missed Calls', missedCalls, missedCalls > 0 ? '#EF9F27' : '#1D9E75')}
        ${metricCell('New Contacts', newContacts != null ? newContacts : '—', '#378ADD')}
      </tr>
    </table>

    <div style="background:#1a1400;border:1px solid #2e2300;border-left:3px solid ${YELLOW};border-radius:4px;padding:18px 20px;margin-bottom:28px;">
      <div style="font-size:0.65rem;color:#888;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px;">Open Pipeline Value</div>
      <div style="font-size:1.8rem;font-weight:900;color:${YELLOW};line-height:1;">${esc(pipelineDisplay)}</div>
      <div style="font-size:0.75rem;color:#555;margin-top:6px;">Active deals in your CRM</div>
    </div>

    ${narrative ? `
    <div style="background:#111;border:1px solid #1f1f1f;border-radius:6px;padding:20px 22px;margin-bottom:28px;">
      <div style="font-size:0.65rem;color:#666;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:12px;">Monthly Analysis</div>
      <p style="font-size:0.875rem;color:#bbb;line-height:1.8;margin:0;white-space:pre-line;">${esc(narrative)}</p>
    </div>
    ` : `
    <p style="font-size:0.85rem;color:#666;margin:0 0 24px;line-height:1.75;">
      Sarah answered every call, 24 hours a day — including nights and weekends.
      Missed calls got a text back automatically. All bookings landed directly on your calendar.
    </p>
    `}

    <a href="${dashUrl}" style="display:inline-block;background:${YELLOW};color:#000;font-weight:700;font-size:0.9rem;padding:13px 28px;border-radius:4px;text-decoration:none;letter-spacing:0.04em;">
      View Full Dashboard →
    </a>

    <p style="font-size:0.78rem;color:#444;margin:24px 0 0;line-height:1.6;">
      Questions about your report? <a href="mailto:riley@thehypeboxllc.com" style="color:${YELLOW};">riley@thehypeboxllc.com</a>
    </p>
  `;

  return {
    subject: `Your ${esc(monthLabel)} Report — ${esc(biz)}`,
    html: wrap(body),
  };
}
