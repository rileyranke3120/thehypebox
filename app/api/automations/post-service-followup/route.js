...
export async function POST(request) {
  ...
  try {
    const { phone_number, customer_name, business_name, client_id } = await request.json();
    ...
    await insertWithRetry(supabase, 'post_service_followups', {
      phone_number,
      customer_name,
      business_name,
      client_id,
      sent_at: new Date().toISOString(),
    }, { tag: '[post-service-followup]' });
