...
export async function POST(request) {
  try {
    ...
    const subscription = await stripe.subscriptions.retrieve(subscriptionId, {
      expand: ['pending_setup_intent', 'customer'],
    });
    ...
    // Check for existing account to avoid duplicates
    ...
    await supabase
      .from('users')
      .update({ password_hash, name: name || email.split('@')[0], plan_status: 'trialing' })
      .eq('email', email.toLowerCase());
    ...
    try {
        ...
        html: `
          ...
          <body style="margin:0;padding:0;background:#0a0a0a;font-family:system-ui,sans-serif;">
            <div style="max-width:560px;margin:0 auto;padding:48px 24px;">
              ...
              <h1 style="font-size:1.75rem;font-weight:800;color:#ffffff;margin:0 0 8px 0;">You're in, ${name ? name.split(' ')[0] : 'friend'}!</h1>
              <p style="font-size:1rem;color:#999;margin:0 0 32px 0;">Your <strong style="color:#FFD000;">${planLabel}</strong> 14-day free trial is now active.</p>
              <div style="background:#111;border:1px solid #222;border-radius:8px;padding:24px;margin-bottom:32px;">
                <p style="font-size:0.75rem;letter-spacing:0.1em;text-transform:uppercase;color:#666;margin:0 0 16px 0;">Your Login Credentials</p>
              ...
              <p style="font-size:0.82rem;color:#555;margin:32px 0 0 0;line-height:1.6;">
                ...
                Questions? <a href="mailto:riley@thehypeboxllc.com" style="color:#FFD000;">riley@thehypeboxllc.com</a>
              </p>
            </div>
          </body>
          </html>
      ...
      console.error('[checkout/confirm] email failed:', emailErr.message);
    }
